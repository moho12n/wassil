...
class _HomePageState extends State<HomePage>
            ...
            SliverFillRemaining(
              child: TabBarView(
                ...
                children: <Widget>[
                  ...
                  Column(children: _buildPosts(Post.localPosts),),
                  /// here is the national post page
                  Column(children: _buildPosts(Post.nationalPosts),),
                ],
              ),
            ),
          ],
  ...
  List<Widget> _buildPosts(List<Post> list) {
    ...
    for (int i = 0; i < list.length; i++) {
      if (list[i] is News) {
        _returnList.add(NewsWidget(news: list[i],));
      }
      else if (list[i] is Poll) {
        _returnList.add(PollWidget(poll: list[i],));
      }
      else if (list[i] is Project) {
        _returnList.add(ProjectWidget(project: list[i],));
      }
    }
    return _returnList;
  }
